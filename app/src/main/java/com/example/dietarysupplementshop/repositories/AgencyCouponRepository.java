...
import com.example.dietarysupplementshop.interfaces.AgencyCouponAPI;
...
public class AgencyCouponRepository {

    private static AgencyCouponRepository instance;
    private final AgencyCouponAPI agencyCouponAPI;
    private final Gson gson;

    private AgencyCouponRepository() {
        this.agencyCouponAPI = RetrofitClient.getRetrofitInstance().create(AgencyCouponAPI.class);
        this.gson = new Gson();
    }

    public static AgencyCouponRepository getInstance() {
        if (instance == null) {
            synchronized (AgencyCouponRepository.class) {
                if (instance == null) {
                    instance = new AgencyCouponRepository();
                }
            }
        }
        return instance;
    ...
    public LiveData<Resource<VoucherCreationResponse>> createCoupon(CreateVoucherRequest request) {
        ...
        agencyCouponAPI.createCoupon(request).enqueue(new Callback<ResponseModel<VoucherCreationResponse>>() {
            @Override
    ...
    public LiveData<Resource<List<Coupon>>> getAgencyCoupons(String status) {
        MutableLiveData<Resource<List<Coupon>>> data = new MutableLiveData<>();
        data.setValue(Resource.loading(null));

        agencyCouponAPI.getAgencyCoupons(status).enqueue(new Callback<ResponseModel<List<Coupon>>>() {
            @Override
    ...
    public LiveData<Resource<VoucherCreationResponse>> updateCoupon(Long couponId, CreateVoucherRequest request) {
        ...
        agencyCouponAPI.updateCoupon(couponId, request).enqueue(new Callback<ResponseModel<VoucherCreationResponse>>() {
            @Override
            public void onResponse(Call<ResponseModel<VoucherCreationResponse>> call, Response<ResponseModel<VoucherCreationResponse>> response) {
                if (response.isSuccessful() && response.body() != null) {
    ...
    public LiveData<Resource<VoucherCreationResponse>> updateCouponStatus(Long couponId, boolean isActive) {
        ...
        agencyCouponAPI.updateCouponStatus(couponId, isActive).enqueue(new Callback<ResponseModel<VoucherCreationResponse>>() {
            @Override
            public void onResponse(Call<ResponseModel<VoucherCreationResponse>> call, Response<ResponseModel<VoucherCreationResponse>> response) {
                if (response.isSuccessful() && response.body() != null) {
    ...
    public LiveData<Resource<Void>> deleteCoupon(Long couponId) {
        ...
        agencyCouponAPI.deleteCoupon(couponId).enqueue(new Callback<ResponseModel<Void>>() {
            @Override
